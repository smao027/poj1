...
import sys
sys.path.append('..')
from Functions.util import (get_tensors_JM, get_mask, init_weights, get_std_opt)
from Functions.loss import (long_loss, surv_loss)
from Functions.metrics import (AUC, Brier, MSE)
from Models.Gate import Transformer2
import numpy as np
from sklearn.preprocessing import MinMaxScaler
torch.manual_seed(0)
def data_preprocessing(source, normalize=True):
    if source == 'JM':
        ##Load Data
        #data = pd.read_pickle('data/DIVAT_sim_1000_visit_1000_long_3_seed_123.pkl')
        #data["cum_hazard"] = data.groupby("id")["Zeta"].cumsum()
        #data["true"] = np.exp(-data["cum_hazard"])
    ...
    return train_id, train_data, test_id, test_data, data


def main(d_long = 3, d_base = 2, pred_steps = 3,
         d_model = 32, nhead = 4, num_decoder_layers = 7,n_epoch = 2, batch_size = 32):
    train_id, train_data, test_id, test_data, data_all = data_preprocessing('JM')
    print("start")
    train_long, train_base, train_mask, e_train, t_train, train_obs_time = get_tensors_JM(train_data.copy(),d_base = d_base)
    ...
    for epoch in range(n_epoch):
        ...
        for batch in range(0, len(train_id), batch_size):
            ...
            batch_long, batch_base, batch_mask, batch_e, batch_t, obs_time = get_tensors_JM(batch_data.copy(),d_base = d_base)
            ...
            print("batch_mask_out shape:", batch_mask_out.shape)
    ...
    plt.close()
    print('Training complete. Loss values saved to loss.png')
    ##Evaluation
    landmark_times = [1,2,3,4,5]
    pred_windows = list(range(1, pred_steps + 1))
    n_sim = 1
    ...
    for LT_index, LT in enumerate(landmark_times):
        ...
        #true_prob_tmp = tmp_all.loc[tmp_all["visit"].isin(pred_times), ["true"]].values.reshape(-1,len(pred_times))
        #true_prob_tmp = tmp_all.loc[tmp_all["predtime"].isin(pred_times), ["true"]].values.reshape(-1,len(pred_times))
        #true_prob_LT = tmp_all.loc[tmp_all["visit"]==LT, ["true"]].values
        ...
        tmp_long, tmp_base, tmp_mask, e_tmp, t_tmp, obs_time = get_tensors_JM(tmp_data.copy(), d_base = d_base)
        ...
        for pt in pred_times:
            ...
            long_out,surv_out = model.mmoe_layer(out[:,-1,:].unsqueeze(1), out, tmp_mask.unsqueeze(1), torch.tensor(pt))
        ...
        AUC_array[0, LT_index, :] = auc
        iAUC_array[0, LT_index] = iauc
        ...
        BS_array[0, LT_index, :] = bs
        iBS_array[0, LT_index] = ibs
        #bs, ibs = Brier(true_prob_tmp, e_tmp.numpy(), t_tmp.numpy(),
        #                    e_train.numpy(), t_train.numpy(), LT, np.array(pred_windows))
        #true_BS_array[0, LT_index, :] = bs
        #true_iBS_array[0, LT_index] = ibs
    
    #Longitudinal Prediction
    test_long, test_base, test_mask, e_test, t_test, obs_time = get_tensors_JM(test_data.copy(),d_base = d_base)    
    ...
    for j in range(1,test_long.shape[1]):
        ...
        out = model.decoder(dec_long.to('cuda'), dec_base.to('cuda'), 
                            get_mask(test_mask[:,:j]).to('cuda'), obs_time[:,:j].to('cuda'))
        long_out,surv_out = model.mmoe_layer(out[:,-1,:].unsqueeze(1).to('cuda'), 
                                             out, test_mask[:,:j].unsqueeze(1).to('cuda'), 
                                             obs_time[:,j].unsqueeze(1).to('cuda'))
    ...
    print(long_mse)
    with open("../Results/summary_metrics.txt", "w") as f:
        f.write("AUC Score (Model):\n")
        f.write(np.array2string(AUC_array[0], precision=4, separator=", ") + "\n\n")
        f.write("Integrated AUC Score (Model):\n")
        f.write(np.array2string(iAUC_array[0], precision=4, separator=", ") + "\n\n")
        f.write("Brier Score (Model):\n")
        f.write(np.array2string(BS_array[0], precision=4, separator=", ") + "\n\n")
        f.write("Integrated Brier Score (Model):\n")
        f.write(np.array2string(iBS_array[0], precision=4, separator=", ") + "\n\n")
        
        f.write("Longitudinal MSE:\n")
        f.write(np.array2string(long_mse[0], precision=4, separator=", ") + "\n")


    print("evaluation done")

if __name__ == "__main__":
    main(d_base = 2, pred_steps = 3, n_epoch = 50)